...
const createResponse = async (req, res) => {
    try {
        ...
        const createdAtDate = reviewData.createdAt ? reviewData.createdAt.toDate ? reviewData.createdAt.toDate() : new Date(reviewData.createdAt) : new Date();
        const responseTimeMinutes = Math.max(0, Math.floor((respondedAt.getTime() - createdAtDate.getTime()) / (1000 * 60)));
        ...
        await AuditService.logEvent({
            actorUid: respondedBy,
            action: 'REVIEW_RESPONSE_SUBMITTED',
            targetId: newResponseRef.id,
            targetType: 'response',
            branchId: reviewData.branchId,
            metadata: { reviewId, responseTimeMinutes }
        });
